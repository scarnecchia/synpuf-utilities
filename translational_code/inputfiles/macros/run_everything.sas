...
%*  1.1.0          11/12/2020 DC        - add OUTDS_scdm_pvd, OUTDS_scdm_fac, and               *;
%*                                        OUTDS_scdm_enr parameters to final_processing call    *;
%************************************************************************************************;


%macro run_everything(startnum=, endnum=);
    ...
    %do s=&startnum %to &endnum;
        ...
        %FINAL_PROCESSING(    INDS_scdm_enr     = scdm.enrollment_&s 
                            ...
                            , OUTDS_scdm_pvd    = scdm.provider_&s 
                            , OUTDS_scdm_fac    = scdm.facility_&s
        ...
        %* Unless running in QC mode, delete temporary datasets from the workspace              *;
        %if %upcase(&YN_QCMode) ne Y %then %do;
